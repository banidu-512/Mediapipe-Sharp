...
public partial class MainForm : Form
{
    ...
    private bool _faceDetectorAvailable;
    ...
    private bool IsValidModelFile(string modelPath)
    {
        try
        {
            ...
            {
                Console.WriteLine(
                    $"[IsValidModelFile] File too small: {fileInfo.Length} bytes (expected at least 1MB)");
                return false;
            }
    ...
    private void MainForm_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            // Set up form
            Text = _demoMode ? "Face Detection - Demo Mode (No Face Detection)" : "Face Detection - Mediapipe";
    ...
    private void ProcessFrame(Mat frame)
    {
        try
        {
            ...
            {
                DetectionResultItem detection = result.Detections[faceIndex];
                ...
                if (detection.Keypoints != null)
                    for (int i = 0; i < Math.Min(6, detection.Keypoints.Count); i++)
                    {
                        ...
                        Console.WriteLine(
                            $"[ProcessFrame]   Keypoint {i}: X={keypoint.X:F3}, Y={keypoint.Y:F3}, Score={keypoint.Score}");
                    }
            }
