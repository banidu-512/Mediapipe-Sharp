...
A Windows Forms application that demonstrates real-time face detection using Mediapipe library and OpenCV for webcam capture.
...
The application requires `blaze_face_short_range.tflite` model file to be placed in Models directory for face detection functionality. This file contains the trained face detection model.

**Important**: The application includes a placeholder file that is NOT a valid model. You must copy the actual model file to enable face detection.
