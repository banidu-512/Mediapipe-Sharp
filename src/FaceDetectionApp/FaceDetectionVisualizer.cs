...
namespace FaceDetectionApp
{
    ...
    public static class FaceDetectionVisualizer
    {
            ...
            bool showLabels = true)
        {
            ...
            var bboxColor = boxColor ?? new Scalar(0, 255, 0); // Green for bounding boxes
            var kpColor = keypointColor ?? new Scalar(0, 0, 255); // Red for keypoints
            
            Console.WriteLine($"[DrawFaceDetection] Processing {result.Detections.Count} face detections");

            foreach (var detection in result.Detections)
            {
                Console.WriteLine($"[DrawFaceDetection] Processing face detection with {detection.Keypoints?.Count ?? 0} keypoints");
        ...
        /// </summary>
        private static void DrawKeypoints(Mat frame, DetectionResultItem detection, Scalar color, int radius, bool showLabels)
        {
