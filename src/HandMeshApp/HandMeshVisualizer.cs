...
public static class HandMeshVisualizer
{
    ...
    private static readonly int[][] HandMeshConnections = new[]
    {
        // Wrist
        ...
        int connectionThickness = 1)
    {
        ...
        foreach (NormalizedLandmarks handLandmarks in result.HandLandmarks)
        {
    ...
    {
        Console.WriteLine(
            $"[DrawConnections] Drawing {HandMeshConnections.Length} connections with thickness {thickness}");
        int drawnConnections = 0;
        int skippedConnections = 0;
        int invalidConnections = 0;

        foreach (int[] connection in HandMeshConnections)
