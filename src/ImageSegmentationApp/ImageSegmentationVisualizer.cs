    ...
    public static class ImageSegmentationVisualizer
    {
        ...
        private static void DrawCategoryMask(Mat frame, Image categoryMask, double alpha)
        {
            try
            {
                ...
                using (var maskMat = ConvertImageToMat(categoryMask))
                {
                    if (maskMat == null) 
                    {
                    ...
                    using (var resizedMask = new Mat())
                    {
                        ...
                        using (var coloredMask = new Mat(frame.Size(), MatType.CV_8UC3, new Scalar(0, 255, 0)))
                        {
        ...
        private static void DrawConfidenceMask(Mat frame, Image confidenceMask, double alpha)
        {
            try
            {
                ...
                using (var maskMat = ConvertImageToMat(confidenceMask))
                {
                    if (maskMat == null) 
                    {
        ...
        private static void DrawSegmentationSummary(Mat frame, ImageSegmenterResult result, Scalar? textColor = null)
        {
            try
            {
                ...
                Cv2.PutText(frame, summary, new OpenCvSharp.Point(10, frame.Height - 20), 
        ...
        private static Mat ConvertImageToMat(Image image)
        {
            try
            {
                ...
                var matType = format.Equals(ImageFormat.Vec32F1) ? MatType.CV_32FC1 :
