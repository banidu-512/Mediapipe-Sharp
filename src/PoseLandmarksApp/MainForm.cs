...
namespace PoseLandmarksApp
{
    public partial class MainForm : Form
    {
        ...
        private bool _poseLandmarkerAvailable = false;
        ...
        private bool IsValidModelFile(string modelPath)
        {
            try
            {
                ...
                {
                    Console.WriteLine($"[IsValidModelFile] File too small: {fileInfo.Length} bytes (expected at least 10MB)");
                    return false;
                }
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                // Set up form
                this.Text = _demoMode ? "Pose Landmarks Detection - Demo Mode (No Pose Detection)" : "Pose Landmarks Detection - Mediapipe";
        ...
        private void ProcessFrame(Mat frame)
        {
            try
            {
                ...
                var result = _poseLandmarker.DetectForVideo(image, _frameTimestamp);
                ...
                for (int poseIndex = 0; poseIndex < result.PoseLandmarks.Count; poseIndex++)
                {
                    ...
                    Console.WriteLine($"[ProcessFrame]   Visible landmarks (vis > 0.3): {visibleCount}/{poseLandmarks.landmarks.Count}");
                }
