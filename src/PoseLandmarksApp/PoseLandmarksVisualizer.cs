...
namespace PoseLandmarksApp
{
    ...
    public static class PoseLandmarksVisualizer
    {
        ...
        private static readonly Scalar[] BodyPartColors = new Scalar[]
        {
            ...
            int connectionThickness = 2)
        {
            ...
            var lmColor = landmarkColor ?? new Scalar(0, 255, 255); // Yellow for landmarks
            var connColor = connectionColor ?? new Scalar(0, 255, 0); // Green for connections
            ...
            for (int poseIndex = 0; poseIndex < result.PoseLandmarks.Count; poseIndex++)
            {
                ...
                Console.WriteLine($"[DrawPoseLandmarks] Processing pose {poseIndex} with {poseLandmarks.landmarks.Count} landmarks");
        ...
        /// </summary>
        private static void DrawLandmarks(Mat frame, NormalizedLandmarks poseLandmarks, Scalar color, int radius, int poseIndex)
        {
            ...
            for (int i = 0; i < poseLandmarks.landmarks.Count; i++)
            {
                ...
                {
                    shouldDraw = landmark.Visibility.Value > 0.3f;
                }
                
                if (landmark.Presence.HasValue && landmark.Presence.Value < 0.3f)
                {
                    shouldDraw = false;
                }
        ...
        private static void DrawConnections(Mat frame, NormalizedLandmarks poseLandmarks, Scalar color, int thickness)
        {
            ...
            foreach (var connection in PoseConnections)
            {
                ...
                {
                    var startLandmark = poseLandmarks.landmarks[connection[0]];
                    var endLandmark = poseLandmarks.landmarks[connection[1]];
                    ...
                    {
                        startVisible = startLandmark.Visibility.Value > 0.3f;
                    }
                    if (startLandmark.Presence.HasValue)
                    {
                        startVisible = startVisible && startLandmark.Presence.Value > 0.3f;
                    }
                    
                    if (endLandmark.Visibility.HasValue)
                    {
                        endVisible = endLandmark.Visibility.Value > 0.3f;
                    }
                    if (endLandmark.Presence.HasValue)
                    {
                        endVisible = endVisible && endLandmark.Presence.Value > 0.3f;
                    }
        ...
        private static Scalar GetLandmarkColor(int landmarkIndex)
        {
            ...
            else if (landmarkIndex >= 11 && landmarkIndex <= 22) // Arms and hands
                return landmarkIndex % 2 == 1 ? BodyPartColors[2] : BodyPartColors[3]; // Right vs Left
            else if (landmarkIndex >= 23 && landmarkIndex <= 32) // Legs and feet
        ...
        private static void DrawPoseConfidence(Mat frame, NormalizedLandmarks poseLandmarks, int poseIndex)
        {
            try
            {
                ...
                if (visibleCount > 0)
                {
                    ...
                    {
                        var nose = poseLandmarks.landmarks[0];
        ...
        /// </summary>
        public static void DrawPoseBoundingBox(Mat frame, NormalizedLandmarks poseLandmarks, Scalar color, int thickness = 2)
        {
            ...
            foreach (var landmark in poseLandmarks.landmarks)
            {
                ...
                if (landmark.Visibility.HasValue)
                {
                    isVisible = landmark.Visibility.Value > 0.3f;
                }
