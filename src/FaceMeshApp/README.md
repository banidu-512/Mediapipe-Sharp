...
A Windows Forms application that demonstrates real-time face mesh detection using Mediapipe library and OpenCV for
webcam capture.
...
The application requires `face_landmarker.task` model file to be placed in Models directory for face detection
functionality. This file contains the trained face landmark detection model.

**Important**: The application includes a placeholder file that is NOT a valid model. You must download the actual model
...
4. Place the file in the `FaceMeshApp/Models/` directory, replacing the existing placeholder
