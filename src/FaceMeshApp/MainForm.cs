...
namespace FaceMeshApp
{
    public partial class MainForm : Form
    {
        ...
        private bool _faceLandmarkerAvailable = false;
        ...
        private bool IsValidModelFile(string modelPath)
        {
            try
            {
                ...
                {
                    Console.WriteLine($"[IsValidModelFile] File too small: {fileInfo.Length} bytes (expected at least 3MB)");
                    return false;
                }
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                // Set up form
                this.Text = _demoMode ? "Face Mesh Detection - Demo Mode (No Face Detection)" : "Face Mesh Detection - Mediapipe";
        ...
        private void ProcessFrame(Mat frame)
        {
            try
            {
                ...
                var result = _faceLandmarker.DetectForVideo(image, _frameTimestamp);
                ...
                if (result.FaceLandmarks != null && result.FaceLandmarks.Count > 0)
                {
                    ...
                    {
                        var faceLandmarks = result.FaceLandmarks[faceIndex];
                        ...
                        for (int i = 0; i < sampleCount; i++)
                        {
                            ...
                            Console.WriteLine($"[ProcessFrame]   Landmark {i}: X={landmark.X:F3}, Y={landmark.Y:F3}, Z={landmark.Z:F3}, Visibility={landmark.Visibility}, Presence={landmark.Presence}");
                        }
                        ...
                        Console.WriteLine($"[ProcessFrame]   Visible landmarks (vis > 0.3): {visibleCount}/{faceLandmarks.landmarks.Count}");
                    }
                }
                else
                {
