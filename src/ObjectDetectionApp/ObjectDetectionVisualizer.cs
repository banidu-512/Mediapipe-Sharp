...
public static class ObjectDetectionVisualizer
{
    ...
    private static readonly Scalar DefaultColor = new(255, 255, 255); // White
        ...
        int boxThickness = 2)
    {
        ...
        {
            Console.WriteLine(
                $"[DrawObjectDetection] Processing detection with {detection.Keypoints?.Count ?? 0} keypoints");
            ...
            if (showLabels || showConfidence)
                DrawLabelAndConfidence(frame, detection, categoryName, confidence, showLabels, showConfidence,
                    boxColor);
        }
    ...
    /// </summary>
    private static void DrawLabelAndConfidence(Mat frame, DetectionResultItem detection, string categoryName,
        float confidence, bool showLabel, bool showConfidence, Scalar color)
    {
        try
        {
            Rect bbox = detection.BoundingBox;
            ...
            // Build label text
            string labelText = "";
            if (showLabel) labelText += categoryName;
            if (showConfidence)
            {
            ...
            if (!string.IsNullOrEmpty(labelText))
            {
                ...
                Size textSize = Cv2.GetTextSize(labelText, HersheyFonts.HersheySimplex, 0.6, 1, out int baseline);

                // Draw background rectangle for better visibility
                Point labelTopLeft = new(x, y - textSize.Height - 5);
                Point labelBottomRight = new(x + textSize.Width, y + 5);
