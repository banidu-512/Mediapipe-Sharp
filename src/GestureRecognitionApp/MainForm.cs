...
public partial class MainForm : Form
{
    ...
    private readonly object _gestureHistoryLock = new();
    ...
    private bool IsValidModelFile(string modelPath)
    {
        try
        {
            ...
            {
                Console.WriteLine(
                    $"[IsValidModelFile] File too small: {fileInfo.Length} bytes (expected at least 5MB)");
                return false;
            }
    ...
    private void MainForm_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            // Set up form
            Text = _demoMode
                ? "Gesture Recognition - Demo Mode (No Gesture Recognition)"
    ...
    private void ProcessFrame(Mat frame)
    {
        try
        {
            ...
            List<GestureRecognizer.GestureResult> gestureResults = new();
            for (int handIndex = 0; handIndex < result.HandLandmarks.Count; handIndex++)
            {
                ...
                List<Classifications>? handedness = result.Handedness != null && result.Handedness.Count > handIndex
                    ? new List<Classifications> { result.Handedness[handIndex] }
                    : null;

                GestureRecognizer.GestureResult gestureResult =
                    GestureRecognizer.RecognizeGesture(handLandmarks, handedness);
                gestureResults.Add(gestureResult);

                Console.WriteLine(
                    $"[ProcessFrame] Hand {handIndex}: {gestureResult.Description} (Confidence: {gestureResult.Confidence:F2})");
            }
    ...
    private void UpdateGestureHistory(List<GestureRecognizer.GestureResult> currentGestures)
    {
        lock (_gestureHistoryLock)
        {
            ...
            foreach (GestureRecognizer.GestureResult gesture in currentGestures) _gestureHistory.Add(gesture);

            // Keep only last 100 gesture results
            if (_gestureHistory.Count > 100) _gestureHistory.RemoveRange(0, _gestureHistory.Count - 100);
        }
    }
    ...
    private string GetStatusMessage(List<GestureRecognizer.GestureResult> gestureResults)
    {
        ...
        List<GestureRecognizer.GestureResult> recognizedGestures =
            gestureResults.Where(g => g.Gesture != GestureRecognizer.GestureType.Unknown).ToList();
        ...
        foreach (GestureRecognizer.GestureResult gesture in recognizedGestures)
            gestureCounts[gesture.Gesture] = gestureCounts.GetValueOrDefault(gesture.Gesture, 0) + 1;

        // Build status message
        List<string> gestureNames = new();
        foreach (KeyValuePair<GestureRecognizer.GestureType, int> kvp in gestureCounts.OrderByDescending(x => x.Value))
        {
