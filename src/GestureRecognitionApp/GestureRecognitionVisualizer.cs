...
public static class GestureRecognitionVisualizer
{
    ...
    private static readonly int[][] HandConnections = new[]
    {
        // Thumb
        ...
        bool showLabels = true)
    {
        ...
        Scalar lmColor = landmarkColor ?? new Scalar(0, 255, 0); // Green for landmarks
        Scalar connColor = connectionColor ?? new Scalar(0, 0, 255); // Blue for connections
        ...
        for (int handIndex = 0; handIndex < result.HandLandmarks.Count; handIndex++)
        {
            ...
            GestureRecognizer.GestureResult? gestureResult = gestureResults != null && gestureResults.Count > handIndex
                ? gestureResults[handIndex]
                : null;

            Console.WriteLine(
                $"[DrawHandGestures] Processing hand {handIndex} with {handLandmarks.landmarks.Count} landmarks");
    ...
    private static void DrawLandmarks(Mat frame, NormalizedLandmarks handLandmarks, Scalar color, int radius)
    {
        ...
        foreach (NormalizedLandmark landmark in handLandmarks.landmarks)
        {
            ...
            if (landmark.Visibility.HasValue) shouldDraw = landmark.Visibility.Value > 0.1f;
            ...
            else
            {
                ...
                if (skippedCount <= 5) // Log first 5 skipped landmarks for debugging
                    Console.WriteLine(
                        $"[DrawLandmarks] Skipped landmark: X={landmark.X:F3}, Y={landmark.Y:F3}, Visibility={landmark.Visibility}");
            }
        }
    ...
    private static void DrawConnections(Mat frame, NormalizedLandmarks handLandmarks, Scalar color, int thickness)
    {
        ...
        foreach (int[] connection in HandConnections)
            ...
            {
                NormalizedLandmark startLandmark = handLandmarks.landmarks[connection[0]];
                NormalizedLandmark endLandmark = handLandmarks.landmarks[connection[1]];

                // Only draw if both landmarks are visible enough
                bool startVisible = true;
                bool endVisible = true;

                if (startLandmark.Visibility.HasValue) startVisible = startLandmark.Visibility.Value > 0.1f;

                if (endLandmark.Visibility.HasValue) endVisible = endLandmark.Visibility.Value > 0.1f;
    ...
    /// </summary>
    private static void DrawGestureLabel(Mat frame, NormalizedLandmarks handLandmarks,
        GestureRecognizer.GestureResult gestureResult)
    {
        try
        {
            ...
            {
                bool isVisible = true;
                if (landmark.Visibility.HasValue) isVisible = landmark.Visibility.Value > 0.1f;
            ...
            if (visibleCount > 0)
            {
                ...
                // Get gesture color
                Scalar gestureColor = GestureColors.ContainsKey(gestureResult.Gesture)
                    ? GestureColors[gestureResult.Gesture]
                    : new Scalar(255, 255, 255);

                // Prepare label text
                string labelText = $"{gestureResult.Description}";
                if (gestureResult.Confidence > 0) labelText += $" ({gestureResult.Confidence:F2})";
                ...
                Size textSize = Cv2.GetTextSize(labelText, HersheyFonts.HersheySimplex, 0.6, 1, out _);
                Rect rect = new(
                    labelX - 5,
                ...
                Cv2.Rectangle(frame, rect, new Scalar(0, 0, 0), -1); // Black background
                Cv2.Rectangle(frame, rect, gestureColor, 2); // Colored border

                // Draw text
                Cv2.PutText(frame, labelText, new Point(labelX, labelY),
                    HersheyFonts.HersheySimplex, 0.6, gestureColor);
    ...
    /// </summary>
    public static void DrawHandBoundingBox(Mat frame, NormalizedLandmarks handLandmarks, Scalar color,
        int thickness = 2)
    {
    ...
    /// </summary>
    public static void DrawGestureSummary(Mat frame, List<GestureRecognizer.GestureResult>? gestureResults,
        Scalar? textColor = null)
    {
        try
        {
            ...
            // Count gesture types
            Dictionary<GestureRecognizer.GestureType, int> gestureCounts = new();
            foreach (GestureRecognizer.GestureResult result in gestureResults)
            ...
            foreach (KeyValuePair<GestureRecognizer.GestureType, int> kvp in gestureCounts.OrderByDescending(x =>
                         x.Value))
            {
                string gestureName = kvp.Key.ToString();
                if (kvp.Value > 1) gestureName += $" (x{kvp.Value})";
            ...
            Rect summaryRect = new(10, frame.Height - totalHeight - 10, 300, totalHeight);

            Cv2.Rectangle(frame, summaryRect, new Scalar(0, 0, 0), -1); // Black background
            Cv2.Rectangle(frame, summaryRect, color); // Border

            // Draw text lines
            for (int i = 0; i < summaryLines.Count; i++)
            {
                Point textPoint = new(15, frame.Height - totalHeight + 5 + (i + 1) * lineHeight);
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(status))
                Cv2.PutText(frame, status, new Point(10, 60),
                    HersheyFonts.HersheySimplex, 0.6, color);
