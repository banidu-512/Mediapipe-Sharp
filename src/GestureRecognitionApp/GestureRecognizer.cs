...
public class GestureRecognizer
{
    ...
    public static GestureResult RecognizeGesture(NormalizedLandmarks handLandmarks, List<Classifications>? handedness)
    {
        if (handLandmarks.landmarks.Equals(null) || handLandmarks.landmarks.Count < 21)
            return new GestureResult
                { Gesture = GestureType.Unknown, Confidence = 0f, Description = "No hand detected" };

        NormalizedLandmark[] landmarks = handLandmarks.landmarks.ToArray();
        bool isLeftHand = DetermineHandedness(handedness);

        // Check various gestures in order of specificity
        List<GestureResult> results = new();
        ...
        GestureResult bestResult = results.OrderByDescending(r => r.Confidence).First();
        if (bestResult.Confidence < 0.3f)
            return new GestureResult
            {
                Gesture = GestureType.Unknown, Confidence = bestResult.Confidence, Description = "Unknown gesture",
    ...
    private static bool DetermineHandedness(List<Classifications>? handedness)
    {
        ...
        {
            Classifications classification = handedness.First();
    ...
    private static bool IsFingerExtended(NormalizedLandmark[] landmarks, int tipIndex, int pipIndex, int mcpIndex)
    {
        ...
        NormalizedLandmark mcp = landmarks[mcpIndex];
        NormalizedLandmark wrist = landmarks[WRIST];
    ...
    {
        NormalizedLandmark thumbTip = landmarks[THUMB_TIP];
        NormalizedLandmark wrist = landmarks[WRIST];
    ...
    {
        NormalizedLandmark thumbTip = landmarks[THUMB_TIP];
        NormalizedLandmark indexTip = landmarks[INDEX_FINGER_TIP];
    ...
    {
        NormalizedLandmark thumbTip = landmarks[THUMB_TIP];
        NormalizedLandmark pinkyTip = landmarks[PINKY_TIP];
        NormalizedLandmark pinkyMcp = landmarks[PINKY_MCP];
    ...
    private static GestureResult CheckHeartSign(NormalizedLandmark[] landmarks, bool isLeftHand)
    {
        NormalizedLandmark thumbTip = landmarks[THUMB_TIP];
        NormalizedLandmark indexTip = landmarks[INDEX_FINGER_TIP];
        NormalizedLandmark middleTip = landmarks[MIDDLE_FINGER_TIP];
